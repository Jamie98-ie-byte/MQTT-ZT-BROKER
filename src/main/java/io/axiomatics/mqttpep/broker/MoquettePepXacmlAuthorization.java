...
public class MoquettePepXacmlAuthorization implements IAuthorizatorPolicy {
    ...
    private boolean isActionAllowed(String clientId, String resource, String action) {
        try {
            long requestStartTime = System.currentTimeMillis(); // Log the start time
            System.out.println("Request to PDP started at: " + new java.util.Date(requestStartTime));
            ...
            long responseTime = System.currentTimeMillis(); // Log the response time
            System.out.println("Response from PDP received at: " + new java.util.Date(responseTime));
            System.out.println("Time taken for authorization request: " + (responseTime - requestStartTime) + " ms");
    ...
    private Request buildXacmlRequest(String clientId, String resource, String action) {
        ...
        subject.addAttribute(new Attribute("mqtt.clientId", clientId));  // Add clientId as an attribute
        Category resourceCategory = new Category();
        resourceCategory.addAttribute(new Attribute("mqtt.topic", resource));  // Add resource (topic) as an attribute

        // Create the action category for the XACML request
        Category actionCategory = new Category();
        actionCategory.addAttribute(new Attribute("mqtt.actionId", action));  // Add action (publish/subscribe) as an attribute
    ...
    public boolean canWrite(Topic topic, String user, String client) {
        ...
        return isActionAllowed(client,topic.toString(), "publish");  // Check authorization for publishing
    }
    ...
    public boolean canRead(Topic topic, String user, String client) {
                ...
                return isActionAllowed(client, topic.toString(), "subscribe");  // Check authorization for subscribing
    }
}
